...
public class MainActivity5 extends AppCompatActivity {
    int InputSize = 8;
    String model_name = "quantized_inference_model_int8.tflite";
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        newtext = new TextView(MainActivity5.this);
    ...
    public void ClickedHuman(View view){
        first = "Human";
        Toast.makeText(getApplicationContext(), "You chose Human", Toast.LENGTH_SHORT).show();
    }

    public void ClickedComputer(View view){
        first = "Computer";
        Toast.makeText(getApplicationContext(), "You chose Computer", Toast.LENGTH_SHORT).show();
    }
    ...
    public void Clickedimage(View view){
        ...
        for(int i=0; i<state.legal_actions().size(); i++){
            if(state.legal_actions().get(i) == action){
                is_available = true;
            }
        }
        ...
        board_change(state);
        newtext.setText("Computer is thinking...");
        if(state.is_done()){
            if(state.is_win()){
                newtext.setText("Computer win!");
            }
            if(state.is_lose()){
        ...
        board_change(state);
        while(state.legal_actions().get(0) == 64){
            state = state.next(64);
            if(state.is_done()){
                if(state.is_win()){
                    newtext.setText("Computer win!");
            ...
            best_choice = state.legal_actions().get(max_idx);
            state = state.next(best_choice);
            board_change(state);
        }
        if(state.is_done()){
        ...
        newtext.setText("It's your turn!");
    }

    public void ClickedStart(View view){
        if(first == ""){
            Toast.makeText(getApplicationContext(), "Choose who will be first", Toast.LENGTH_SHORT).show();
            return;
        }
        try{
